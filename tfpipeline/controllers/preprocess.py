    ...
    while True:
        ...
        try:
            for obj in resp['Contents']:
                ...
                #print('getting ' + _key)
                xmlbody = str(_obj['Body'].read().decode('utf-8'))
                e = ET.fromstring(xmlbody)
                fname = os.path.basename(
                    e.find('./filename').text).split('.')[0] + '.tcapture'
                folder = job.project['id'] + '/train'
                _obj = job.s3.get_object(
                    Bucket=job.bucket, Key=folder + '/' + fname)
                jsonbody = str(_obj['Body'].read().decode('utf-8'))
                tcapture = json.loads(jsonbody)
                img_ct = len(tcapture['images'])
                train_sz = int(img_ct * float(job.train_sz))
                test_sz = img_ct - train_sz
                for node in e.iter('object'):
                    xmin = int(node.find('./bndbox/xmin').text)
                    ymin = int(node.find('./bndbox/ymin').text)
                    xmax = int(node.find('./bndbox/xmax').text)
                    ymax = int(node.find('./bndbox/ymax').text)
                    xsz = tcapture['config']['xres']
                    ysz = tcapture['config']['yres']
                    name = node.find('./name').text
                    if name not in job.labelmap_dict:
                        job.labelmap_dict[name] = len(job.labelmap_dict)+1
                    with open(train_csv_file, "a") as fout:
                        for z in range(1):
                            img_pth = folder + '/' + tcapture['images'][z]
                            fout.write(img_pth + ',' + str(xsz) + ',' + str(ysz) +
                                       ','+name + ',' + str(xmin) + ',' + str(ymin) + ',' + str(xmax) + ',' + str(ymax) + '\n')
                    with open(test_csv_file, "a") as fout:
                        for z in range(1):
                            img_pth = folder + '/' + \
                                tcapture['images'][z+train_sz]
                            fout.write(img_pth + ',' + str(xsz) + ',' + str(ysz) +
                                       ','+name + ',' + str(xmin) + ',' + str(ymin) + ',' + str(xmax) + ',' + str(ymax) + '\n')
                if hasattr(job, 'jb'):
