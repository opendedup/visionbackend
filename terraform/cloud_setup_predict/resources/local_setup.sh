...
apt update -y
apt install -y docker-ce docker-ce-cli nvidia-driver-430
...
docker pull flexiblevision/capture-ui:$8
docker pull minio/minio
docker pull flexiblevision/capture:$8
...
docker run -d --name=capdev -p 0.0.0.0:5000:5000 --restart unless-stopped --privileged -v /dev:/dev -v /sys:/sys -e BUCKET=$5 \
    ...
    -d -e "GOOGLE_APPLICATION_CREDENTIALS=/credentials.json" flexiblevision/capture:$8
docker run -p 0.0.0.0:80:3000 --restart unless-stopped -v $HOME/fv_do_not_delete/fvision_creds.json:/credentials.json \
    ...
    -e PROCESS_SERVER=http://$3.$4.c.$2.internal -e GCP_ZONE=$4 -e GCP_PROJECT=$2 -e "GOOGLE_APPLICATION_CREDENTIALS=/credentials.json" flexiblevision/capture-ui:$8
