...
def remove_cloud_instance():
    check_gcp_login()
    p = query_yes_no("Remove the CLOUD INSTACE for Suttle Vision ? ",default="no")
    if p:
...
def remove_local_instance():
    check_gcp_login()
    p = query_yes_no("Remove a LOCAL CAMERA SERVER for Suttle Vision ? ",default="no")
    if p:
        sys.stdout.write("######################      Warning     ########################\n")
...
def deploy_cloud_instance():
    check_gcp_login()
    p = query_yes_no("Setup a cloud instance of Suttle Vision ? ")
    if p:
...
def deploy_local_cam():
    ...
    p = query_yes_no("Deploy a Suttle Vision local camera server?")
    if p:
        sys.stdout.write("######################      Warning     ########################\n")
...
def main():
    sys.stdout.write("Welcome to the Suttle Vision Setup.\n")
    while True:
